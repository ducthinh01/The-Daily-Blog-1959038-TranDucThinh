<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if(empty($errors)) {
            if (!empty($data['image'])) {
                $sql = "UPDATE `posts` SET `title`='". $data['title'] ."',`description`='". $data['description'] ."',`image`= '". $data['image'] ."',`contents`='". $data['contents'] ."' WHERE id=".$id ;
            } else {
                $sql = "UPDATE `posts` SET `title`='". $data['title'] ."',`description`='". $data['description'] ."',`contents`='". $data['contents'] ."' WHERE id=".$id ;
            }
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <div class="col-md-12">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="title" id="title" placeholder="Tile" value="<?= $post['title'] ?>" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea name="contents" id="contents" class="form-control" cols="30" rows="10"><?= $post['contents'] ?></textarea>
                    <script>
                        CKEDITOR.replace('contents');
                    </script>
                </div>
                <button type="submit" class="btn btn-dark" style="margin-bottom: 15px">Update Post</button>
            </div>
        </form>
    </div>
</div>
